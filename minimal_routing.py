...
## the simplest class of router, which only picks one minimal path, and route all traffic there.
class MinimalRouting(object):
	def __init__(self):
	...
	def _find_all_short_paths_to_entry_switches(self, topology, switch_to_block_map, block_to_switches_map, nblocks, inter_block_entrance_switches):
		...
		for current_switch in topology.keys():
			...
			for target_block in range(nblocks):
				...
				paths_collection = []
				if current_switch in inter_block_entrance_switches[(current_switch_block, target_block)]:
					paths_collection.append((current_switch,))
				else:
	...
	def _split_traffic_between_block_pair(self, src_block, dst_block, total_capacity_between_blocks, block_to_switches_map, entry_switches, traffic_sent_from_each_switch, path_to_entry_switches):
	...
	def load_balance(self, topology, switch_to_block_map, block_to_switches_map, nblocks, switch_to_switch_traffic_matrix, inter_block_entrance_switches, path_to_entry_switches):
		...
		for src_block in range(nblocks - 1):
			for dst_block in range(src_block + 1, nblocks, 1):
				...
				all_routing_weights[(src_block, dst_block)] = self._split_traffic_between_block_pair(dst_block, src_block, capacity2, block_to_switches_map, entry_switches2, switch_to_switch_traffic_matrix2, path_to_entry_switches)
