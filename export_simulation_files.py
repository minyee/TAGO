...
def compute_weighted_percentile(array_of_vals, weights, percentile):
	assert(len(array_of_vals) == len(weights))
	...
	cumulative = np.cumsum([x[1] for x in sorted_tuple_pair_vector])
	#print(cumulative)
	percentile_location = cumulative[-1] * float(percentile)
	for i in range(len(sorted_tuple_pair_vector)):
		if cumulative[i] >= percentile_location:
			print i
			return sorted_tuple_pair_vector[i][0]
	return -1
...
def export_topology_performances_different_load_levels(topology_subdirectory, traffic_workload, export_directory):
	...
	p99_rtt_performances = {}
	p999_rtt_performances = {}
	...
	for routing in set_of_routing:
		p99_rtt_performances[routing] = [-1] * len(load_levels)
		p999_rtt_performances[routing] = [-1] * len(load_levels)
	...
	for load_level in load_levels:
		...
		for routing_subdir in routing_subdirs:
			if "results" in routing_subdir[:7]:
				...
				p99_rtt = compute_weighted_percentile(rtts[1], [1] * len(rtts[1]), 0.99)
				p999_rtt = compute_weighted_percentile(rtts[1], [1] * len(rtts[1]), 0.999)
				ave_fct_performances[routing_scheme][offset] = ave_fct
				ave_rtt_performances[routing_scheme][offset] = ave_rtt / 1000.
				p99_rtt_performances[routing_scheme][offset] = p99_rtt / 1000.
				p999_rtt_performances[routing_scheme][offset] = p999_rtt / 1000.
	...
	export_load_level_performance_to_file(export_directory + "/" + "p99_rtt.txt", load_levels, p99_rtt_performances)
	export_load_level_performance_to_file(export_directory + "/" + "p999_rtt.txt", load_levels, p999_rtt_performances)
...
export_topology_performances_different_load_levels(results_subdir + "/" + topology_name, workload_name, performance_summary_subdir + "/" + topology_name + "/" + workload_name) 
